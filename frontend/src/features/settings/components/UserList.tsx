...
import { Plus, Pencil, Trash2, Mail, Lock, User, Briefcase, Check, Store, Power, KeyRound } from 'lucide-react';
...
export function UserList() {
    ...
    const toggleStatusMutation = useMutation({
        mutationFn: async (id: string) => api.patch(`/users/${id}/toggle-status`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['users'] });
    ...
    const handleToggleStatus = (user: any) => {
        const message = user.isActive ? t('users.deactivate_confirm') : t('users.activate_confirm');
        if (confirm(message)) {
                                ...
                                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {!user.isActive && (
                                                <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 border border-gray-300 dark:border-gray-600">
                                                    {t('users.user_inactive')}
                                                </span>
                                            )}
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex justify-end gap-2">
                                            <button
                                                ...
                                                className={`p-1 rounded-md transition-colors ${user.isActive
                                                    ? 'text-gray-400 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/40'
                                                    : 'text-gray-400 hover:text-green-600 dark:hover:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/40'
                                                    }`}
                                                title={user.isActive ? t('users.deactivate_user') : t('users.activate_user')}
                                            >
                                                <Power className="w-4 h-4" />
                                            </button>
                                            <button
                                                onClick={() => handleResetPassword(user)}
                                                className="p-1 text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/40 rounded-md transition-colors"
                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 dark:bg-black/80 flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md border border-gray-200 dark:border-gray-700">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                            {resetPasswordUser && `${resetPasswordUser.firstName} ${resetPasswordUser.lastName}`}
                        </p>
                        <form onSubmit={handlePasswordResetSubmit} className="space-y-4">
                            ...
                            <div className="flex justify-end space-x-3 mt-6">
                                ...
                                >
                                    {resetPasswordMutation.isPending ? t('common.processing') : t('users.reset_password')}
                                </button>
                            </div>
                        </form>
                    </div>
