                    ...
                    <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl overflow-y-auto max-h-[90vh]">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                                            ...
                                            {newSale.items.map((item, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className="px-4 py-2 text-sm">
                                                        <input type="number" min="1" className="w-16 border rounded p-1" value={item.quantity}
                                                            onChange={(e) => {
                                                                const newItems = [...newSale.items];
                                                    ...
                                                    <td className="px-4 py-2 text-sm text-right">{item.quantity * item.unitPrice}</td>
                                                    <td className="px-4 py-2 text-right">
                                                        <button type="button" onClick={() => handleRemoveItem(index)} className="text-red-600 hover:text-red-900">×</button>
                                                    </td>
                                                </tr>
                                            ))}
                                            {newSale.items.length === 0 && (
                                                <tr><td colSpan={5} className="px-4 py-4 text-center text-gray-500">No items added</td></tr>
                                            )}
                                        </tbody>
                            ...
                            <div>
                                ...
                                <textarea className="mt-1 block w-full border border-gray-300 rounded-md p-2" rows={2}
                                    value={newSale.notes} onChange={(e) => setNewSale({ ...newSale, notes: e.target.value })} />
                            </div>
                            ...
                            <div className="flex justify-end space-x-3 mt-6">
                                ...
                                <button type="submit" disabled={createSaleMutation.isPending || newSale.items.length === 0} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50">
